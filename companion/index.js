...
/* from https://raw.github.com/Skipstone/Skipstone/master/js/Message.js */
var wsMessageQueue = {
  queue: [],
  add: function(obj) {
    this.queue.push(obj);
...
function onOpen(event) {
  ...
  peerSocket.send({_connected:true});
}
...
function onClose(event) {
  ...
  peerSocket.send({_connected:false});
}
...
peerSocket.onmessage = function(e) {
  console.log('Message received from Pebble: ' + JSON.stringify(e.data));

  // if not connect, connect, then send all queued messages
  var command = formatCommand(e.data);
